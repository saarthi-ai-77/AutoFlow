...
class MigrationManager {
    ...
    `;
        ...
        logger_1.logger.info('Migration history table ensured');
    }
    ...
    async applyMigration(migration) {
        try {
            ...
            // Execute migration
            await connection_1.pool.query(migration.up);
            // Record migration as applied
            const insertQuery = `
        INSERT INTO ${this.tableName} (version, name)
        VALUES ($1, $2)
      `;
            await connection_1.pool.query(insertQuery, [migration.version, migration.name]);
    ...
    async rollbackMigration(version) {
        ...
        try {
            await connection_1.pool.query('BEGIN');
            // Execute rollback
            await connection_1.pool.query(migration.down);
            // Remove from migration history
            const deleteQuery = `DELETE FROM ${this.tableName} WHERE version = $1`;
            await connection_1.pool.query(deleteQuery, [version]);
            await connection_1.pool.query('COMMIT');
            logger_1.logger.info(`Rolled back migration: ${version}`);
        }
        catch (error) {
            await connection_1.pool.query('ROLLBACK');
            logger_1.logger.error(`Failed to rollback migration: ${version}`, { error });
            throw error;
        }
    }
}
...
if (require.main === module) {
    ...
    exports.migrationManager.migrate()
        .then(() => {
